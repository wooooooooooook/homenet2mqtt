...
| `kocom` | `packages/core/config/examples/kocom.homenet_bridge.yaml` | 9600 8N1, `0xAA55` 헤더, add 체크섬 | 다구역 조명, 난방 | 조명 명령은 다른 스위치 상태를 참조하도록 Lambda 사용 |
...
| `samsung_sds_door` | `packages/core/config/examples/samsung_sds_door.homenet_bridge.yaml` | 9600 8E1 | 도어벨 바이너리 센서, 문자 센서 | XOR 기반 람다 체크섬과 응답 상태 파서 포함 |
