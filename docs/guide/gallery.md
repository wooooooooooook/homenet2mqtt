...
본 문서는 Homenet2MQTT의 **스니펫 갤러리**에 등록될 설정 조각(Snippet)을 직접 제작하려는 개발자와 사용자를 위한 기술 가이드입니다. 

직접 제작하신 훌륭한 스니펫이 있다면 [GitHub Pull Request](https://github.com/wooooooooooook/homenet2mqtt/pulls)를 통해 기여해 주세요! 여러분의 공유가 커뮤니티에 큰 도움이 됩니다.
...
## 요구사항 매칭 (Requirements)

스니펫이 특정 하드웨어 설정(Baud Rate, Parity 등)이나 패킷 기본 설정과 호환되는지 확인하기 위해 `requirements.json` 파일을 사용할 수 있습니다.

### 매칭 규칙

기존의 단순 일치 매칭 외에도 범위(Range)와 목록(List) 매칭을 지원합니다.

#### 1. 범위 매칭 (Range)
숫자 값에 대해 최소값(`min`)과 최대값(`max`)을 지정할 수 있습니다.
```json
{
  "serial": {
    "baud_rate": { "min": 3800, "max": 3899 }
  }
}
```

#### 2. 목록 매칭 (List)
허용 가능한 여러 값을 배열로 지정할 수 있습니다.
```json
{
  "serial": {
    "parity": ["none", "even"],
...
{
  "serial": {
    "baud_rate": { "min": 9500, "max": 9700 },
    "parity": ["none", "even"],
    "data_bits": 8
  },
  "packet_defaults": {
    "rx_checksum": ["none", "crc16"]
  }
}
```
