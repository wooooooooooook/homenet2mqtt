...
describe('DiscoveryManager', () => {
    ...
    beforeEach(() => {
        ...
        mockConfig = {
            ...
            switch: [
                { id: 'switch1', name: 'Test Switch', type: 'switch', state: {} }
            ],
            sensor: [
                { id: 'sensor1', name: 'Test Sensor', type: 'sensor', device_class: 'temperature', unit_of_measurement: '°C', state: {} }
            ],
            climate: [
                { id: 'climate1', name: 'Test Climate', type: 'climate', state: {} }
            ]
        } as any;
    ...
    it('should publish discovery config for switch', () => {
        ...
        const call = mockPublisher.publish.mock.calls.find((args: any[]) => args[0] === 'homeassistant/switch/homenet_switch1/config');
    ...
    it('should publish discovery config for sensor with device_class', () => {
        ...
        const call = mockPublisher.publish.mock.calls.find((args: any[]) => args[0] === 'homeassistant/sensor/homenet_sensor1/config');
        const payload = JSON.parse(call[1]);
    ...
    it('should publish discovery config for climate', () => {
        ...
        const call = mockPublisher.publish.mock.calls.find((args: any[]) => args[0] === 'homeassistant/climate/homenet_climate1/config');
        const payload = JSON.parse(call[1]);
        ...
        expect(payload.action_topic).toBe('homenet/climate1/state');
        expect(payload.action_template).toBe('{{ value_json.action }}');
