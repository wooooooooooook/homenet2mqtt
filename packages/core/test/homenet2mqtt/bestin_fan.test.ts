...
describe('Bestin Fan Test', () => {
    ...
    it('should parse fan status packets correctly', async () => {
        ...
        if (!fanEntity) {
            console.log('ventilation_fan 엔티티가 없습니다. 이 테스트는 bestin2.0.homenet_bridge.yaml에 fan 엔티티가 있어야 합니다.');
    ...
    it('should verify fan command packet structure matches specification', () => {
        ...
        const powerOnData = celExecutor.execute(
            '[0x61, 0x01, 10, 0x00, 0x01, 0x01, 0x00, 0x00]',
            {}
        );
        ...
        const speed3Data = celExecutor.execute(
            '[0x61, 0x03, 10, 0x00, 0x00, 3, 0x00, 0x00]',
            {}
        );
        ...
        const naturalOnData = celExecutor.execute(
            '[0x61, 0x07, 10, 0x00, 0x10, 0x00, 0x00, 0x00]',
            {}
        );
