...
describe('Bestin Climate Test', () => {
    ...
    it('should parse heating status packets for all rooms', async () => {
        ...
        if (!room1Entity) {
            console.log('room1_heating 엔티티가 없습니다. 이 테스트는 bestin2.0.homenet_bridge.yaml에 climate 엔티티가 있어야 합니다.');
        ...
        const packetRoom1 = [0x02, 0x28, 0x10, 0x91, 0xA8, 0x01, 0x11, 0x58, 0x00, 0xf2, 0x00, 0x25, 0x00, 0x00, 0x00];
    ...
    it('should test bitwise CEL functions for temperature decoding', () => {
        ...
        const result3 = celExecutor.execute('bitOr(bitShiftLeft(data[8], 8), data[9])', { data: packet });
