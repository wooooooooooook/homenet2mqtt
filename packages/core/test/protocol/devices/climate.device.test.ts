...
describe('ClimateDevice Command Generation', () => {
  ...
  it('should succeed when using complex list structure with "x" (Kocom Pattern)', () => {
      const config = {
      name: 'test_climate',
      command_temperature: '[[0x30, 0xbc, 0x00, 0x36, 0x03, 0x01, 0x00, 0x00, 0x11, 0x01, x, 0x00, 0x00, 0x00, 0x00, 0x00], [0x30, 0xdc]]',
    };

    const device = new ClimateDevice(config, protocolConfig);
    const command = device.constructCommand('temperature', 20);

    expect(command).toEqual([0x30, 0xbc, 0x00, 0x36, 0x03, 0x01, 0x00, 0x00, 0x11, 0x01, 20, 0x00, 0x00, 0x00, 0x00, 0x00]);
  });
